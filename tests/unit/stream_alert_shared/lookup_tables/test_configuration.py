...
class TestLookupTablesConfiguration(object):
        ...
        return {
            ...
            "global": {
                "infrastructure": {
                    "lookup_tables": {
                        "buckets": {
                            "airbnb.sample.lookuptable": [
                                "resource_map_legacy.gz"
                            ]
                        },
                        "cache_refresh_minutes": 10,
                        "enabled": True
    ...
    def test_configuration_table_names(self):
        ...
        assert_equal(
            config.table_names,
            ['resource_map_prototype', 'resource_map_dynamodb', 'resource_map_legacy']
        )
    ...
    def test_configuration_table_configurations(self):
        ...
        assert_equal(config.table_configurations['resource_map_legacy']['driver'], 's3')
    ...
    def test_configuration_conflict_configuration(self):
        """LookupTablesConfiguration - Conflict Configuration - Raises Exception"""
        config = {
            "lookup_tables": {
                "enabled": True,
                "tables": {
                ...
                "infrastructure": {
                    "lookup_tables": {
                        "buckets": {
                            "airbnb.sample.lookuptable": [
                                "resource_map_legacy.gz",
                                "resource_map_prototype.gz",  # Duplicate
                            ]
                        },
                        "cache_refresh_minutes": 10,
                        "enabled": True
        ...
        assert_raises(Exception, LookupTablesConfiguration, config)
