...
class DeployCommand(object):
    ...
    def handler(cls, options, config):
        ...
        if not cls.execute_lambda_deployment(functions, config, options.clusters):
            return False

        # Update the rule table now if the rules engine is being deployed
        if 'rule' in functions:
            cls._update_rule_table(options, config)

        return True

    @classmethod
    def execute_lambda_deployment(cls, functions, config, clusters=None):
        ...
        for function in functions:
            package, targets = cls._create(function, config, clusters)
    ...
    @classmethod
    def _update_rule_table(cls, options, config):
    ...
    @classmethod
    def _create(cls, function_name, config, clusters=None):
        """
        Args:
...
def deploy(functions, config, clusters=None):
    ...
    """
    return DeployCommand.execute_lambda_deployment(functions, config, clusters=clusters)
