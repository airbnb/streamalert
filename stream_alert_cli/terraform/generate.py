...
from stream_alert_cli.athena.handler import get_athena_database_name
...
def generate_athena_settings(config, config_name, generate_func, tf_tmp_file, message):
    """
    Args:
    ...
    """
    if not config['global']['infrastructure'].get(config_name):
        LOGGER.warning('Config for \'%s\' not in global.json', config_name)
        remove_temp_terraform_file(tf_tmp_file, message)
        return

    if config['global']['infrastructure'][config_name].get('enabled', True):
