...
def generate_global_lambda_settings(config, **kwargs):
    ...
    Args:
        ...
        **kwargs: includes following keys:
    ...
    """
    if config['lambda'].get('config_name') and kwargs.get('tf_tmp_file'):
        tf_tmp_file = kwargs['tf_tmp_file']
        if config['lambda']['config_name']['enabled']:
