...
def cloudtrail_mfa_policy_abuse_attempt(rec):
    """
    author:           spiper of Summit Route in collaboration with Duo Security
    description:      Alert on potential attacks performed by users that are supposed to have
                        MFA enforced. May indicate leaked access keys and an attempt to abuse a flawed
                        MFA enforcement policy.
    playbook:         (a) Identify whether or not the attempt was a mistake.
                      (b) Begin IR: Roll access keys, investigate past CloudTrail logs for other actions performed, investigate how the keys were leaked in the first place.
    """
    ...
    try:
        mfaAuthenticated = rec['userIdentity']['sessionContext']['attributes']['mfaAuthenticated']
    except KeyError:
        pass
    
    # If the user is MFA authenticated, then any issues are not due to just a compromised access key,
    # so ignore it.
    ...
    if (rec['eventName'] == "CreateVirtualMFADevice" or rec['eventName'] == 'EnableMFADevice'):
        if 'errorCode' in rec:
            return True
