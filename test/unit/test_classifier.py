...
class TestStreamPayload(object):
    ...
    def test_classify_record_kinesis_json(self):
        ...
        assert_equal(type(payload.record[0]), dict)
        ...
        # record type test
        assert_equal(type(payload.record[0]['key1']), str)
        assert_equal(type(payload.record[0]['key2']), str)
        assert_equal(type(payload.record[0]['key3']), int)
    ...
    def test_classify_record_kinesis_nested_json(self):
        ...
        # record type test
        assert_equal(type(payload.record[0]['date']), str)
        assert_equal(type(payload.record[0]['unixtime']), int)
        assert_equal(type(payload.record[0]['data']), dict)

        # record value test
        assert_equal(payload.record[0]['date'], 'Jan 01 2017')
        assert_equal(payload.record[0]['data']['key1'], 'test')
    ...
    def test_classify_record_kinesis_nested_json_osquery(self):
        ...
        # record type test
        assert_equal(type(payload.record[0]['hostIdentifier']), str)
        assert_equal(type(payload.record[0]['unixTime']), int)
        assert_equal(type(payload.record[0]['columns']), dict)
        assert_equal(type(payload.record[0]['decorations']), dict)

        # record value test
        assert_equal(payload.record[0]['unixTime'], 1485556524)
        assert_equal(payload.record[0]['columns']['key1'], 'test')
        assert_equal(payload.record[0]['decorations']['cluster'], 'eu-east')
        assert_equal(payload.record[0]['decorations']['number'], 100)
    ...
    def test_classify_record_kinesis_nested_json_with_data(self):
        ...
        assert_equal(type(payload.record[0]['unixtime']), int)
        assert_equal(type(payload.record[0]['data']), dict)
        assert_equal(type(payload.record[0]['data']['type']), int)
        assert_equal(type(payload.record[0]['data']['category']), str)

        # record value test
        assert_equal(payload.record[0]['date'], 'Jan 01 2017')
        assert_equal(payload.record[0]['data']['source'], 'dev-app-1')
    ...
    def test_classify_record_kinesis_csv(self):
        ...
        # record value tests
        assert_equal(payload.record[0]['message'],
                     'thisis some data with keyword1 in it')
        assert_equal(payload.record[0]['host'], 'host1')
    ...
    def test_classify_record_kinesis_csv_nested(self):
        ...
        # record value tests
        assert_equal(payload.record[0]['date'], 'Jan 10 2017')
        assert_equal(payload.record[0]['host'], 'host1.prod.test')
        assert_equal(payload.record[0]['time'], 1485635414)
        assert_equal(payload.record[0]['message']['role'], 'web-server')
        assert_equal(payload.record[0]['message']['cluster_size'], 10)
    ...
    def test_classify_record_kinesis_kv(self):
        ...
        # record value tests
        assert_equal(payload.record[0]['type'], 'SYSCALL')
        assert_equal(payload.record[0]['suid'], 500)
        assert_equal(payload.record[0]['pid'], 3538)
        assert_equal(payload.record[0]['type_3'], 'PATH')
    ...
    def test_classify_record_syslog(self):
        ...
        for name, syslog_message in fixtures.iteritems():
            ...
            # record value tests
            if name == 'test_1':
                assert_equal(payload.record[0]['host'], 'vagrant-ubuntu-trusty-64')
                assert_equal(payload.record[0]['application'], 'sudo')
                assert_equal(payload.record[0]['message'], 'pam_unix(sudo:session): session opened for user root by (uid=0)')
            elif name == 'test_2':
                assert_equal(payload.record[0]['host'], 'macbook004154test')
                assert_equal(payload.record[0]['application'], 'authd')
